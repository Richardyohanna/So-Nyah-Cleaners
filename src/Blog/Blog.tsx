...
function SkeletonCard() {
  return (
    <div className="w-full border border-[#0000001a]  overflow-hidden bg-white animate-pulse">
...
const Blog = () => {
  ...
  return (
    ...
    <>
      ...
      <div className="overflow-x-hidden bg-white px-4 py-6 pb-16 sm:px-6 sm:py-8 sm:pb-20 md:px-8 lg:px-10 lg:py-10">
        ...
        <section id="blogs" className="mt-6 sm:mt-8 sm:mt-10">
          ...
          {isFiltered && !loading && (
            <div className="mb-5 flex items-center gap-3 flex-wrap">
              ...
              <button
                ...
                className="text-xs text-[var(--primary)] border border-[var(--primary)]/30 px-3 py-1 hover:bg-[var(--primary)] hover:text-white transition-all duration-200"
              >
                Clear filters ✕
              </button>
            </div>
...
>>>>>>> 16c80a74640db8449e755433c41774f07b3a18a7
                ...
                <div className="p-5 sm:p-6 lg:p-7 flex flex-col gap-3 min-h-[240px]">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {article.hashtags?.slice(0, 2).map((tag) => (
                      <button
                        ...
                        className="text-[10px] sm:text-xs tracking-wide uppercase font-medium px-2.5 py-1 bg-[rgba(246,243,242,1)] text-[var(--primary)] hover:bg-[var(--primary)] hover:text-white transition-colors duration-200"
                      >
                        {tag}
                      </button>
                    ))}
                  ...
                  <div className="flex gap-3 items-center mt-auto pt-2">
                    <div className="w-10 h-10 bg-[rgba(246,243,242,1)] flex items-center justify-center shrink-0">
          ...
          {!loading && totalPages > 1 && (
            <section className="flex gap-3 sm:gap-5 justify-center items-center mt-10 sm:mt-12 flex-wrap">
              <button onClick={() => goTo(Math.max(1, currentPage - 1))} disabled={currentPage === 1}
                className="border border-[var(--primary)] w-12 h-12 sm:w-14 sm:h-14 flex items-center justify-center transition-all duration-300 hover:bg-[var(--primary)] hover:scale-105 group disabled:opacity-40 disabled:cursor-not-allowed">
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                <button key={page} onClick={() => goTo(page)}
                  className={`w-12 h-12 sm:w-14 sm:h-14 flex items-center justify-center font-semibold transition-all duration-300 hover:scale-105 ${
                    page === currentPage
          ...
          <section className="mt-12 sm:mt-14 lg:mt-16">
            <div className="relative overflow-hidden  bg-[var(--primary)] px-5 sm:px-8 md:px-10 lg:px-14 py-8 sm:py-10 lg:py-12 text-white">
              <div className="absolute top-0 right-0 w-[180px] sm:w-[240px] h-[180px] sm:h-[240px] bg-white/10 blur-3xl -translate-y-1/3 translate-x-1/4 pointer-events-none" />
              <div className="absolute bottom-0 left-0 w-[160px] sm:w-[220px] h-[160px] sm:h-[220px] bg-white/10 blur-3xl translate-y-1/3 -translate-x-1/4 pointer-events-none" />
              <div className="relative z-10 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-8">
                <div className="max-w-[620px]">
                  <span className="inline-block bg-white/15 border border-white/20 px-4 py-2 text-xs sm:text-sm tracking-wide uppercase mb-4">Newsletter</span>
                  ...
                  <div className="mt-5 flex flex-wrap gap-3 text-xs sm:text-sm text-white/85">
                    <span className="bg-white/10 border border-white/15 px-3 py-2 ">Weekly tips</span>
                    <span className="bg-white/10 border border-white/15 px-3 py-2 ">No spam</span>
                    <span className="bg-white/10 border border-white/15 px-3 py-2 ">Unsubscribe anytime</span>
                  </div>
                </div>
