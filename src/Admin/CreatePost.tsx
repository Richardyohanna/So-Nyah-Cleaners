...
interface FormData {
  ...
  existingImageUrl: string | null; // url already stored in supabase
  hashtags: string[];
  hashtagInput: string;
  meta: string;
}
...
const ToolbarBtn = ({
  ...
  <button
    ...
    title={title}
    className={`flex items-center justify-center min-w-[28px] h-[28px] px-[6px] py-[4px] rounded-[5px] text-[12px] transition cursor-pointer ${
  ...
  return (
    <div>
      <div
        ...
        onDrop={handleDrop}
        className={`border-2 border-dashed rounded-[10px] overflow-hidden transition-all duration-200 min-h-[140px] flex items-center justify-center relative
      ...
      {preview && (
        <div className="flex gap-3 mt-2">
          ...
          <button onClick={onRemove} className="text-xs text-red-500 hover:underline cursor-pointer">
            Remove
          </button>
          <input ref={inputRef} type="file" accept="image/*" className="hidden"
            onChange={(e) => { const f = e.target.files?.[0]; if (f) onFile(f); }} />
...
// ─── Sidebar cards (reused in both layouts) ───────────────────────────────────
...
export default function CreatePost({ editPost = null, onSaved }: Props) {
  ...
  useEffect(() => {
    if (editPost) {
      setForm({
        ...
        introduction: editPost.introduction,
        image: null,
        imagePreview: editPost.image_url,
        existingImageUrl: editPost.image_url,
        hashtags: editPost.hashtags ?? [],
        hashtagInput: "",
        meta: editPost.meta ?? "",
      });
      // editor HTML is set inside RichEditor via initialHTML prop
    } else {
      setForm({
        title: "",
        introduction: "",
  ...
  const handleImage = (file: File) => {
    ...
    reader.onload = (e) =>
      setForm((f) => ({ ...f, image: file, imagePreview: e.target?.result as string, existingImageUrl: null }));
  ...
  const uploadImage = async (file: File): Promise<string | null> => {
    ...
    if (error) {
      console.error("Storage upload error:", error.message, error); // 👈 add this
      return null;
    }
  ...
  return (
    <div className="font-[Georgia,serif]">
      ...
      {/* action buttons row — shown inside content area */}
      <div className="flex justify-end gap-2 mb-4">
        {saveError && (
          <p className="text-xs text-red-500 self-center mr-auto">{saveError}</p>
        )}
        <button
          onClick={() => save("draft")}
          disabled={saving}
      ...
      <div className="flex flex-col lg:flex-row gap-5 lg:gap-6 max-w-[1200px] mx-auto">
        ...
        <div className="flex-1 flex flex-col gap-4 md:gap-5 min-w-0">
          ...
          {/* sidebar cards — mobile/tablet only */}
